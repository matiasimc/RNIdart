...
class BodyMethodVisitor extends RecursiveAstVisitor {
  final Logger log = new Logger("BodyFunctionVisitor");
  LocalEnvironment localEnv;
  GlobalEnvironment env;
  MethodElement method;

  BodyMethodVisitor(this.env, this.localEnv, this.method);
  ...
  visitMethodInvocation(MethodInvocation node) {
    ...
    if (target is SimpleIdentifier) {
      ...
      if (target.bestElement.computeNode() is FormalParameter) {
        this.method.parameterUsage[target.bestElement.computeNode()].methodCalls.add(node);
      }
  ...
  visitPrefixedIdentifier(PrefixedIdentifier node) {
    ...
    if (target is SimpleIdentifier) {
      // If it's a parameter
      if (target.bestElement.computeNode() is FormalParameter) {
        this.method.parameterUsage[target.bestElement.computeNode()].fieldCalls.add(node);
      }
      // Else, could be a global variable
      else if (this.env.variables.containsKey(target.name)) {
